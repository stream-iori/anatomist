...
class CallSiteProjectionTest {
        ...
        try (SqliteStore store = new SqliteStore(tmp.resolve("index.db"))) {
            ...
            try (Statement statement = c.createStatement()) {
                ...
                statement.execute("CREATE TRIGGER audit_call_site_delete AFTER DELETE ON call_sites "
                        + "BEGIN INSERT INTO deleted_callers VALUES(old.caller_id); END");
            }
            String unrelatedHash = scalar(c,
                    "SELECT hex(stable_hash) FROM call_sites WHERE caller_id='caller-b'");
            CallSiteProjection.AffectedScope scope = CallSiteProjection.captureAffected(
                    c, List.of("A.java"), Set.of());
            try (Statement statement = c.createStatement()) {
                statement.executeUpdate("DELETE FROM edges WHERE source_file='A.java'");
            }
            CallSiteProjection.refresh(c, scope);
            ...
            assertEquals(unrelatedHash, scalar(c,
                    "SELECT hex(stable_hash) FROM call_sites WHERE caller_id='caller-b'"));
            assertEquals(0, count(c, "SELECT count(*) FROM call_sites WHERE caller_id='caller-a'"));
    ...
    void queryIndexesCoverOutgoingAndIncomingLookups(@TempDir Path tmp) throws Exception {
        try (SqliteStore store = new SqliteStore(tmp.resolve("index.db"))) {
            ...
            Connection c = store.connection();
            assertPlanUses(c,
                    "SELECT site_pk FROM call_sites WHERE caller_id=? "
                            + "ORDER BY source_file,begin_line,begin_column,ordinal",
