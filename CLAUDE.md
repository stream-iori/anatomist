...
| Call chain with block slicing | `java -jar target/anatomist.jar callees-of com.example.shop.service.OrderService#createOrder --depth 3 --blocks=package --index /tmp/x.db` |
...
- `query/` — Read-only query layer over the built SQLite index. `QueryService` is the single SQL-bearing class; `QueryEnvelope` / `NodeRow` / `EdgeRow` / `ContextResult` / `HierarchyResult` / `OverviewResult` + `PackageStat` / `BlockResult` / `SliceResult` are result POJOs; `JsonFormatter` + hand-written `DtoCodecs` own the snake_case serialisation (no Jackson). `QueryService.overview()` (kind/edge/package tallies + package-dep skeleton) and `classDepsInternal(maxEdges)` (class→class internal edges, with a recursive-CONTAINS rollup so lambdas / method-refs / anon bodies fold up to their named type) power the `overview` and `export` commands. `callsFrom`/`callsTo`/`callPath` use Java-side BFS that crosses OVERRIDES edges to pierce interface/abstract-method dispatch: callees-of walks implementations downward, callers-of bridges implementations upward to their interface callers, call-path traverses both. OVERRIDES edges are materialized in the output with `relation="OVERRIDES"` so the Agent sees dispatch hops explicitly. `CallChainSlicer` post-processes a call chain into blocks grouped by owning class (Level CLASS) or package+annotation role (Level PACKAGE); each `BlockResult` carries methods, owning types, inbound/outbound/internal edges, field reads/writes, annotations, depth range, and control-flow context. Activated by `--blocks[=class|package]` on callees-of / callers-of / call-path commands. Never imports `com.github.javaparser.*`.
...
- `cli/` — `AnatomistCli` (picocli root) + `IndexCommand` / `IndexDocsCommand` / `WatchCommand` (index-side) + query subcommands (`SearchCommand` / `ContextCommand` / `CalleesOfCommand` / `CallersOfCommand` / `HierarchyCommand` / `ImplementorsOfCommand` / `DepsOfCommand` / `UsedByCommand` / `FieldAccessCommand` / `CallPathCommand` / `AnnotateCommand`) + project-overview commands `OverviewCommand` (top-down summary, `--format markdown|json`, `--depth N`, `--deps-only`) and `ExportCommand` (`--format html --output f.html`, `--max-edges`). `ContextCommand` doubles as the enrich entry point via `--enrich` (adds semantic annotations, docs, suggested queries; supports `--package` for package-level enrichment). `CalleesOfCommand` / `CallersOfCommand` / `CallPathCommand` support `--blocks[=class|package]` to slice call chains into architectural blocks. Query subcommands resolve the index db via `IndexPath` (explicit `--index` flag → fallback `./.anatomist/index.db`).
