...
- `query/` — Read-only query layer over the built SQLite index. `QueryService` is the single SQL-bearing class; `QueryEnvelope` / `NodeRow` / `EdgeRow` / `ContextResult` / `HierarchyResult` / `OverviewResult` + `PackageStat` are Jackson-serializable result POJOs; `JsonFormatter` owns the snake_case + INDENT_OUTPUT mapper. `QueryService.overview()` (kind/edge/package tallies + package-dep skeleton) and `classDepsInternal(maxEdges)` (class→class internal edges, with a recursive-CONTAINS rollup so lambdas / method-refs / anon bodies fold up to their named type) power the `overview` and `export` commands. `callsFrom`/`callsTo`/`callPath` use Java-side BFS that crosses OVERRIDES edges to pierce interface/abstract-method dispatch: callees-of walks implementations downward, callers-of bridges implementations upward to their interface callers, call-path traverses both. OVERRIDES edges are materialized in the output with `relation="OVERRIDES"` so the Agent sees dispatch hops explicitly. Never imports `com.github.javaparser.*`.
